...
- 词表 **288 条**（G/H/I 当前环境主体 + 高置信旧标竞技名），成员校验 0 缺席，gap 标记 6 名（いれかえカート / エール団の応援 / エネルギーサーチ / パワーグラス / ファイトメモリー / ファイアメモリー——官方实有、TCGdex JA 2026-08-14 快照缺席；エネルギーサーチプロ 当日查明正体 = エネルギー転送PRO，SV7a-052 锚定入库，移出 gap）
- TCGdex JA 重抓：8,159 → **12,619 张 / 115 系列**（原 5 缺口覆盖 ギフトエネルギー/ネジキ/野盗三姉妹；いれかえカート/エール団の応援 仍缺席 → gap 标记）
- 回填：name_ja **+1,563 张**（trainer 非空 1,519 / 特殊能量非空 44），全库 name_ja 9,480 → **11,043**；conflicts=0，幂等复跑 0 填充
- GHI 环境口径：distinct EN 名 257 → 覆盖 **218（84.8%）**，未覆盖 39 名全部入 question（多为低置信/竞技冷名：四张 berry、Amarys、Grabber 等），无桥卡 15 张
- 未填充 question 1,336 → 1,319 卡张分类：trainer_vocab_miss 1,196 / energy_vocab_miss 81 / no_en_bridge 42（简中独占促销等）——**零「不知道是什么」项**
...
- **当日人工核对回环（用户在场）**：①Fire Memory 疑点查明——CSM1cC-131 战斗存储碟 / 132 火焰存储碟 官方卡表确为两个不同名，mik 仅 131 的 EN 桥误标（正身 Fighting Memory）；两条 cn 消歧入词表（ファイトメモリー / ファイアメモリー，TCGdex 缺席入 gap 清单）；②Energy Search Pro 正体查明 = エネルギー転送PRO（SV7a-052，TCGdex 锚定通过，移出 gap）；③Energy Switch 名字级取 SV 期名 エネルギーつけかえ 的口径经用户拍板维持现状
...
- 601 测试全绿（589→601）、ruff 全净；词表人工核对回环后 ja_trainer 7 条定向复跑全绿（测试用 tmp fixture 不消费真实词表，真实词表由 load_trainer_vocab 运行时 fail-fast 校验：288 条 0 缺席）
- 回填前备份 `.scratch/ptcg-cn-before-task036-20260814.db`；幂等复跑 0 填充 0 冲突；终版报告由备份还原后单次回填生成（name_ja=1,563 / questions=1,319 / vocab_unused=0）
...
- question 清单 1,319 条可随时经词表追加清偿（开放词表惯例）
- GHI 未覆盖 39 名中若有个别进入竞技视野（如 Scoop Up Cyclone），按需补词表
- gap 标记 6 名待人肉核销（用户认领，2026-08-15）：いれかえカート / エール団の応援（官方 deck 页已见正名，复核即可）/ エネルギーサーチ / パワーグラス / ファイトメモリー / ファイアメモリー（TCGdex 缺席，需官方卡查确认拼写）；核销失败则删条目回 question，不猜
