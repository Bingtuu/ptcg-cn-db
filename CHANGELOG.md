...
- A3 卡面人工比对核销（task 020，2026-08-10，用户在场）：62/62 一致率 100%（旧清单 50 张 + 补充批 12 张——太晶×5 / ACE SPEC×3 / 超梦V-UNION 四部件×4，旧清单缺三类口径按 PRD A3 补抽）；太晶补验关闭——task 030 ptcd 判定的 is_tera 经 9 张人工确认（累计 12 例零误判）；**V-UNION 部件方位回填**——`ptcgdb/normalize/union_positions.py` + CLI `seed-union-positions`（mik 源无方位字段，A3 实测 CSEC 组 4 张连续编号按 card_id 序 = 左上/右上/左下/右下，超梦组逐张实测 + 用户确认全系列同构；幂等、既有不同值不覆盖记 conflicts），真实库 CSEC 20 张已填、validate V-UNION 完整性 checked=24 failures=0；SSP-109~112 同日用户核对同序后回填，24 张方位齐全；核销记录 `reports/sampling-a3-20260802.md`
