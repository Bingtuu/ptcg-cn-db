...
- task 038 Phase 3 效果粗粒度标签层词表定稿（PRD v1.22）：`config/vocabularies/effect_tags.yml`（28 意图标签 + 3 机制 flag，开放追加；初版 23，GHI 实测零命中浮出 5 新类别 coin_manipulate/bench_attack/win_condition/special_summon/counter_shift_self 经用户拍板纯 yml 追加零代码）+ `ptcgdb/mapping/effect_tags.py` loader/matcher/scan（fail-fast、零内置词）+ CLI `tag-effects-scan` 命中率评测（只读零写入）+ `reports/tag-effects-scan-standard-standard-2026-07-16-2026-08-17-20260817.md` GHI 实测报告（1,507 文本覆盖 88.7%，零命中 171 条全归类为口径内无需打标）。
- JP 对齐二期卡级管线（M10-2，task 037，PRD v1.21，migration 012 / user_version=12）：**壳采集**——`scrapers/pokecabook.py`（分类档翻页断点续传、`category_endpoint()` 端点单一事实源、限速 ≥2s、首页 slug 枚举比对防 PJCS 静默漏收）+ `scrapers/pokecardlab.py`（互核通道，日期型 URL）+ `config/jp_tournament_rules.yml` 新单一事实源（分类 slug 档 + `title_tier_overrides` 标题 override：PJCS 无独立 slug 混在 champions 分类）+ `scrapers/jp_rules.py` fail-fast 校验；**deck confirm 定向采集**（FR-9.5 红线放宽首次实战）——`scrapers/deck_confirm.py`：请求量估算器（EstimateReport.total_codes / by_category）+ 闸门判定（默认 500，超限降级 champions-only）+ 计划快照 `data/raw/pokemon-card-jp/plan.json`（decision/window/selected_codes/gate）+ 5s/请求 + 解析熔断（连续 ≥3 次 DeckConfirmParseError abort）+ 逐码断点续传 + 请求台账 `request-ledger.jsonl`（行=逻辑请求，ts=limiter 放行墙钟戳 `HttpClient.last_dispatch_at`）；**卡表解析** `normalize/deck_confirm.py`（8 分组 hidden input + searchItemName 名表 + ACE SPEC 后缀剥离 + `(SET 编号/总数)` 剥除 + 60 张质量门）；**ingest** `normalize/ingest_jp.py`（tournament=一个聚合站 event、date=发布日、tier=slug 档+标题 override、JA 名→`cards.name_ja` 名字级映射 + **多候选同 name_group 裁决**（`ja_name+group_env` env 收窄 → `ja_name+group_latest` 最新印刷，跨组维持 ambiguous 不猜）、miss_kind JP 三档 no_ja_name_match/ambiguous_ja_name/unknown_card_id、record/player_ref NULL 不猜、topcut_slots=入库出战条数物化、plan.json 降级过滤、窗口守卫 FR-9.8 同构）；`deck_misses.remap_decks` 扩展支持 JP 源；CLI 三命令 `scrape jp-shells [--source]` / `scrape jp-decks [--gate N] [--dry-run]` / `ingest-jp [--no-enforce-window]`；**runner 兜底清偿**——limitless_site/limitless/tournament/pokecabook/pokecardlab 五 runner 补 TransientHttpError 顶层兜底（重试耗尽记 question + aborted 保 finish_run）。实跑（降级 champions-only，用户拍板）：deck confirm 229/229 ok（估算 30,935 码 > 500 闸门）、tournaments=106（pjcs 18/cl 88）/ decks=229 / 卡级映射 96.3% / full=157（68.6%，Mega 前月段 99.2%）/ misses 240 行=38 名全 Mega 时代 no_ja_name_match；pokecardlab 互核因反爬门禁用户拍板跳过；报告 `reports/task037-jp-pipeline-20260816.md`
- trainer/特殊能量 name_ja 补强（M10-1，task 036，PRD v1.20，无 schema 迁移）：名字级人工词表种子 `config/vocabularies/ja_trainer_names.yml`（290 条，EN 主键 + 可选 cn 消歧 + `tcgdex_gap` 豁免 2 名均已用户官方卡查核销，校验锚 = JA 名 ∈ TCGdex JA 名表 fail-fast）+ `ptcgdb/mapping/ja_trainer.py`（`normalize_ja_deck_name` 剥 ACE SPEC 全/半角后缀 / `load_trainer_vocab` / `fill_ja_trainer` 名字级幂等回填、冲突保留原值、cn 消歧未覆盖组成员如实入 question）+ CLI `map-ja-trainer` + `mapping/report.py` 追加 `write_ja_trainer_report`；TCGdex JA 重抓 8,159→12,619 张/115 系列；实测 name_ja **9,480→11,046（+1,566，conflicts=0，幂等复跑 0 填充）**，GHI 环境 distinct EN 名覆盖 218/257=84.8%，未覆盖 1,316 卡张全量归类（trainer_vocab_miss 1,193 / energy_vocab_miss 81 / no_en_bridge 42）；task 035 演练复跑窗口期 5/5 卡组 full（Mega 对照组 partial=no_cn_printing 预期）；报告 `reports/task036-ja-trainer-20260815.md` + `reports/mapping-ja-trainer-20260815.md`

- A2/A3 卡面人工比对收官（task 020 DONE，里程碑 M8，2026-08-10，用户在场）：A2 复查确认 100/100（1,100 项次全勾，2026-08-03 全 10 批完成，差异全为 F-01/F-02/F-03 已知口径、task 030 已清偿）；A3 62/62 一致率 100%（旧清单 50 张 + 补充批 12 张——太晶×5 / ACE SPEC×3 / 超梦V-UNION 四部件×4，旧清单缺三类口径按 PRD A3 补抽）；太晶补验关闭——task 030 ptcd 判定的 is_tera 经 9 张人工确认（累计 12 例零误判）；**V-UNION 部件方位回填**——`ptcgdb/normalize/union_positions.py` + CLI `seed-union-positions`（mik 源无方位字段，A3 实测 4 张连续编号按 card_id 序 = 左上/右上/左下/右下；幂等、既有不同值不覆盖记 conflicts），CSEC 20 张 + SSP 4 张（同日核对）全部回填，24 张方位齐全、validate V-UNION 完整性 0 失败；核销记录 `reports/sampling-a2-20260801.md` / `reports/sampling-a3-20260802.md`
...
- PRD 升 v1.21（task 037）：FR-9.1a 对齐窗口泛化（`alignment_window(region)` 按赛区取值、段匹配改超集语义）+ JP 聚合站通道落地口径（source=pokemon_card_jp / basis=jp / 标题 override / 同 name_group 裁决）+ FR-9.5 成本守卫落采集计划快照 plan.json + FR-9.4 人数因子中性化与 B 层 participant_count 要求（见下两条）
- 赛事静态权重口径（task 037，migration 012 / user_version=12）：`v_tournament_weights` 静态权重件改 `participant_count IS NULL → tier_coef 单因子`（人数因子中性化，不猜人数）——JP 聚合站通道不携带参赛人数（106/106 场 NULL），CN/EN 各源均有人数零漂移（测试锚定）
- B 层统计口径（task 037）：canonical SQL `winrate_b.sql` / `wws.sql` eligible 加 `AND participant_count IS NOT NULL`（top-cut 转化率与 q0 均以人数为分母；JP 只收上位卡组 CR 恒 1 无信息量）——basis=jp 的 WR/WWS 口径性不可用返回空结果，JP 样本仅产 WUR
- tier 词表 aliases 补日文名（task 037）：`config/vocabularies/tournament_tiers.yml` 加 ポケモンジャパンチャンピオンシップス/チャンピオンズリーグ/シティリーグ/City League（tier_coef 数值不变，recaliber 口径无漂移）
